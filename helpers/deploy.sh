...
function invoke_infrastructure_deployer {
  ...
  (eval "$assume_role_exports" && \
    infrastructure-deployer --aws-region "$ECS_DEPLOY_RUNNER_REGION" -- "$container" infrastructure-deploy-script --repo git@github.com:"$GITHUB_REPOSITORY".git --ref "$GITHUB_SHA" --binary "terragrunt" --command "$command" --deploy-path "$CONTEXT")
